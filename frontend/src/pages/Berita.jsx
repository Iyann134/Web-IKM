      ...
      <div className="mx-auto max-w-7xl space-y-12">
        ...
        <div className="rounded-[2rem] border border-[#8b0000]/10 bg-gradient-to-r from-[#fff5ec] via-[#fff0d1] to-[#fff5ec] p-8 md:p-12 shadow-[0_30px_80px_-40px_rgba(0,0,0,0.2)] relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl space-y-4">
            ...
            <h2 className="text-3xl md:text-5xl font-extrabold text-[#8b0000] leading-tight">Berita Terkini IKM ITERA</h2>
                  ...
                  <article key={item.id} className="group flex flex-col overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white shadow-sm hover:shadow-xl hover:border-[#8b0000]/20 transition-all duration-300">
                    ...
                    <div className="p-6 flex flex-col flex-1 justify-between space-y-4">
                      <div className="space-y-2">
                        ...
                        <h4 className="text-xl font-bold text-[#48241f] line-clamp-2 leading-snug group-hover:text-[#8b0000] transition-colors">{item.title}</h4>
                        <p className="text-xs leading-relaxed text-slate-500 line-clamp-3">{item.content}</p>
                      </div>
                      <Link
                        to={`/informasi/berita/${item.id}`}
                        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#8b0000] px-5 py-3 text-xs font-bold text-white transition hover:bg-[#b11919] w-full shadow-sm"
                      >
                        Baca Selengkapnya
